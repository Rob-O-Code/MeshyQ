...
LOG_MODULE_REGISTER(cs47l63_comm, CONFIG_LOG_DEFAULT_LEVEL);

#define CS47L63_DEVID_VAL 0x47A63
#define CS47L63_PAD_LEN 4
#define CS47L63_THREAD_STACK_SIZE 1024
...
static struct k_thread cs47l63_thread_data;
static k_tid_t cs47l63_thread_id;
static K_THREAD_STACK_DEFINE(cs47l63_thread_stack, CS47L63_THREAD_STACK_SIZE);
...
{
	uint8_t pad_buffer[CS47L63_PAD_LEN] = { 0 };
	struct spi_buf rx_buf[] = {
		{ .buf = addr_buffer, .len = addr_length },
	...
	ARG_UNUSED(bsp_dev_id);

	if (pad_len != CS47L63_PAD_LEN) {
		LOG_ERR("Trying to pad more than 4 bytes: %u", pad_len);
...
{
	ARG_UNUSED(supply_id);
	ARG_UNUSED(supply_state);
	return BSP_STATUS_OK;
}
...
{
	ARG_UNUSED(bsp_dev_id);
	ARG_UNUSED(write_buffer);
	ARG_UNUSED(write_length);
	ARG_UNUSED(read_buffer);
...
static uint32_t cs47l63_comm_i2c_db_write(uint32_t bsp_dev_id, uint8_t *write_buffer_0,
					  ...
					  uint32_t write_length_1, bsp_callback_t cb,
					  void *cb_arg)
{
	ARG_UNUSED(bsp_dev_id);
	ARG_UNUSED(write_buffer_0);
	ARG_UNUSED(write_length_0);
	ARG_UNUSED(write_buffer_1);
	ARG_UNUSED(write_length_1);
	ARG_UNUSED(cb);
	ARG_UNUSED(cb_arg);
...
static cs47l63_bsp_config_t bsp_config = {
	.bsp_reset_gpio_id = 18,
	.bsp_int_gpio_id = 19,
	.cp_config.bus_type = CS47L63_BUS_TYPE_SPI,
	.cp_config.spi_pad_len = CS47L63_PAD_LEN,
...
int cs47l63_comm_init(cs47l63_t *cs47l63_driver)
{
	...
	memset(&cs47l63_config, 0, sizeof(cs47l63_config));
	...
	if (!device_is_ready(gpio_dev) || !gpio_is_ready_dt(&hw_codec_gpio) ||
	    !gpio_is_ready_dt(&hw_codec_irq) || !gpio_is_ready_dt(&hw_codec_reset)) {
		LOG_ERR("Codec GPIO dependency is not ready");
		return -ENXIO;
	}
	...
	cs47l63_thread_id = k_thread_create(&cs47l63_thread_data, cs47l63_thread_stack,
					    K_THREAD_STACK_SIZEOF(cs47l63_thread_stack),
					    cs47l63_comm_thread, cs47l63_driver, NULL, NULL,
					    K_PRIO_PREEMPT(CS47L63_THREAD_PRIO), 0,
					    K_MSEC(CS47L63_PROCESS_THREAD_DELAY_MS));

	ret = k_thread_name_set(cs47l63_thread_id, "cs47l63_evt");
	if (ret) {
		return ret;
	}
	...
	if (cs47l63_driver->devid != CS47L63_DEVID_VAL) {
		LOG_ERR("Wrong device id: 0x%05x, should be 0x%05x", cs47l63_driver->devid,
			CS47L63_DEVID_VAL);
		return -EIO;
	}
...
static bsp_driver_if_t bsp_driver_if_s = {
	.set_gpio = &cs47l63_comm_gpio_set,
